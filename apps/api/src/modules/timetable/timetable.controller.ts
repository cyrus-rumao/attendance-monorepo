import Timetable, { ITimetableSlot } from './timetable.model';
...
export const saveTimetable = async (req: Request, res: Response): Promise<Response> => {
  try {
    if (!req.user) {
    ...
    const populated = await Timetable.findOne({ userId })
...
export const getTimetable = async (req: Request, res: Response): Promise<Response> => {
  try {
    if (!req.user) {
      return res.status(401).json({ message: 'Unauthorized' });
    }
...
export const deleteTimetable = async (req: Request, res: Response): Promise<Response> => {
  try {
    if (!req.user) {
      return res.status(401).json({ message: 'Unauthorized' });
    }
