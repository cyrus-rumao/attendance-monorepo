...
export const updateSubject = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const subject = await Subject.findOneAndUpdate({ _id: id, userId }, req.body, { new: true });
...
export const getFullSubjectAnalytics = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const safeBunks = percentage >= 75 ? Math.floor(attended - 0.75 * totalConducted) : 0;

    const hoursNeededToReach75 =
      percentage < 75 ? Math.ceil((0.75 * totalConducted - attended) / 0.25) : 0;
    ...
    return res.status(500).json({ message: 'Failed to fetch subject analytics' });
  }
};
